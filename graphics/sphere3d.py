# -*- coding: utf-8 -*-
"""
Created on Wed Feb 20 17:01:18 2019
...
def sphere_arg0(sol):
    return benchmarks.sphere(sol)[0]
...
X, Y = np.meshgrid(X, Y)
Z = np.fromiter(map(sphere_arg0, zip(X.flat,Y.flat)), dtype=np.float, count=X.shape[0]*X.shape[1]).reshape(X.shape)
# Plot the 3D surface
...
cbar = fig.colorbar(surf, ticks=(np.arange(0,50,5)))
cbar.ax.set_yticklabels(np.arange(0,50,5))
fig.tight_layout()
plt.savefig('sphere.png', dpi=150)
